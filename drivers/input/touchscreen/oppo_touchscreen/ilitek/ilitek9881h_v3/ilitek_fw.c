	...
	static int ilitek_tddi_fw_iram_program(u32 start, u8 *w_buf, u32 w_len, u32 split_len)
	{
		...
		if (split_len != 0) {
			for (addr = start, i = 0; addr < end; addr += split_len, i += split_len) {
				...
				idev->fw_buf_dma[0] = SPI_WRITE;
				idev->fw_buf_dma[1] = 0x25;
				idev->fw_buf_dma[2] = (char)((addr & 0x000000FF));
				idev->fw_buf_dma[3] = (char)((addr & 0x0000FF00) >> 8);
				idev->fw_buf_dma[4] = (char)((addr & 0x00FF0000) >> 16);
	
				for (j = 0; j < split_len; j++)
					idev->fw_buf_dma[5 + j] = w_buf[i + j];
	
				if (idev->spi_write_then_read(idev->spi, idev->fw_buf_dma, split_len + 5, NULL, 0)) {
			...
			if (idev->spi_write_then_read(idev->spi, idev->fw_buf_dma, w_len + 5, NULL, 0) < 0) {
				ipio_err("Failed to write data via SPI in host download (%x)\n", w_len + 5);
				return -EIO;
			}
		}
