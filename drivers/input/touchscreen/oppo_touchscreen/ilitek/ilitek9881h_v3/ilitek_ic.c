...
int ilitek_tddi_ic_get_fw_ver(void)
{
	...
	TPD_INFO("cbs: dev version %d\n", dev_version);

    if (idev->ts->panel_data.manufacture_info.version) {
        u8 ver_len = 0;
		strlcpy(tmp_version, idev->ts->panel_data.manufacture_info.version, idev->ts->panel_data.version_len+1);
...
//            strlcpy(&(idev->ts->panel_data.manufacture_info.version[12]), dev_version, 3);
			strlcat(tmp_version, dev_version, idev->ts->panel_data.version_len +3); 
			strlcpy(idev->ts->panel_data.manufacture_info.version, tmp_version,	idev->ts->panel_data.version_len+3);
        } else {
