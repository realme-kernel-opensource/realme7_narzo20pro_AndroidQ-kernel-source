...
static void core_config_after_write_otp(void)
{
	...
	// DDI Sleep In
	core_config_wr_pack(0x1F100101);
	// Write OFF
	core_config_wr_pack(0x1FFF9500);
	...
	// Write CMD 3C to 00
	core_config_wr_pack(0x1F3C0100);
	// Write OFF
	core_config_wr_pack(0x1FFF9500);
	...
	core_config_wr_pack(0x1FFFFF00);
	// Sleep Out
	core_config_wr_pack(0x1F110101);
	// Display On
...
static int core_config_before_write_otp(void)
{
	...
	core_config_wr_pack(0x1FFFFF00);
	// Write Display Off
	core_config_wr_pack(0x1F280101);
	// Write OFF
	core_config_wr_pack(0x1FFF9500);
	...
	core_config_wr_pack(0x1FFFFF06);
	// Write CMD 3C to 01
	core_config_wr_pack(0x1F3C0101);
	// Write OFF
	core_config_wr_pack(0x1FFF9500);
...
void core_config_otp_read(u8 *otp_data, int data_size, u32 start, int len)
{
	int i;

	ipio_info("data_size = %d, start addr = 0x%x, read_len = %d\n", data_size, 
start, len);
	...
	for (i = start; i < (start + len); i++) {
		...
		ipio_info("Reading OTP data .... %d%c", (((i - start) * 100) / len), '%');
	}

	core_config_ice_mode_write(0x4300C, 0x02, 2);
}

int core_config_otp_write_check(u8 *otp_data, u32 start, int w_len)
{
	int i, ret = 0, retry = 1000;
	u8 prog_one_done = 0x00;

	ipio_info("OTP: start addr = 0x%x, len = %d\n", start, w_len);

	dump_data(otp_data, 8, w_len, 0, "Write OTP");
...
retry);
		...
		for (i = start + 1; i < start + w_len; i++) {
			...
			ipio_info("Writing OTP data .... %d%c", (((i - start) * 100) / w_len), '%');
...
int core_config_otp_copy_bank1_to_bank2(void)
{
	int i;
	int ret = 0;
	...
	core_config_otp_read(buf, otp_max_size, 0x20, 1);
	if (buf[0] == 0xAA) {
		ipio_info("index 0x20 read 0xAA, not do OTP flow\n");
		goto out;
	}
	else {
		ipio_info("index 0x20 read 0x%X, do OTP flow\n", buf[0]);
	}
	...
	dump_data(buf, 8, otp_max_size, 0, "Original OTP");
	...
	dump_data(buf, 8, otp_max_size, 0, "After write OTP");
	ret = 0;
	/* compare result */
	for(i = 0; i < 32; i ++) {
...
int core_config_get_fw_ver(void)
{
	...
	if (ili_ctpmodule == TXD_MODEL) {
		...
		sprintf(core_config->ilitek_ts_info.version, "EA6015%02X-0", core_config->firmware_ver[3]);
	}
	if (protocol->mid >= 0x3) {
	...
	if ( ili_ctpmodule == 0 ) {
	    ...
	    sprintf(ili_version,"INX_Ili_%x",core_config->firmware_ver[3]);
	    devinfo_info_tp_set(ili_version, "INX",OPPO_FIRMWARE_NAME_PATH);
	}
