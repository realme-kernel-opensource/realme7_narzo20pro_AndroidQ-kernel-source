...
int core_config_get_esd_data(void)
{
	...
	txbuf[3] = 0x10;
	txbuf[4] = 0x18;
	//remove core_ice_mode_disable_9881H11()
...
int core_ice_mode_read_9881H11(uint8_t *data, uint32_t size)
{
	...
	txbuf[4] = 0x2;
	if (spi_write_then_read(core_spi->spi, txbuf, 5, txbuf, 0) < 0) {
		ipio_err("146 spi transfer error!\n");
		ret = -EIO;
		return ret;
	}
	...
	if (ret < 0) {
		ipio_err("167 spi transfer error!\n");
	}
...
int core_spi_write(uint8_t *pBuf, uint16_t nSize)
{
	int res = 0;
	uint8_t *txbuf;
	u8 wakeup[10] = {0x82, 0xA3, 0xA3, 0xA3, 0xA3, 0xA3, 0xA3, 0xA3, 0xA3, 0xA3};//mfeng_add
	...
	{
		//mfeng_add start
	        if (ipd->need_dummy_cmd) {
	            ipio_info("wake up spi pll clk\n");
	            if (spi_write_then_read(ipd->spi, wakeup, sizeof(wakeup), NULL, 0) < 0) {
