...
upgrade_fail:
	...
	if (core_fr->actual_fw_mode == protocol->test_mode) {
		...
		/* write hex to the addr of AP code */
		ipio_info("Writing data into AP222 code ...\n");
		if (write_download(0, MAX_AP_FIRMWARE_SIZE, ap_fw, SPI_UPGRADE_LEN) < 0) {
			ipio_err("SPI Write AP code data error\n");
		} else {
			ipio_info("SPI Write AP code data222 ok !!!\n");
		}
...
int core_firmware_upgrade(const char *pFilePath, bool isIRAM)
{
	...
	uint8_t cmd[4] = { 0 };
#endif
   /*chengzhixing for tp shixiao 20200426
	mm_segment_t old_fs;
	loff_t pos = 0;
...
no_hex_file:
	...
	if (core_config->direction == 0) {
		ipio_info("direction = 0,no need send cmd\n");
	} else if (core_config->direction == 1) {
		direc[2] = 0x02;
		core_write(core_config->slave_i2c_addr, direc, 3);
