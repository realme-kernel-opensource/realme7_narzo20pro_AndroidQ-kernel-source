...
#endif
static bool g_flag_self_test_data_init = false;
...
int himax_self_test_data_init(void)
{
	...
	if(g_flag_self_test_data_init == true)
		return ret;/* NOT need to alloc again,return HX_INSPECT_OK*/
	/*
	...
	*/
	...
	for (i = 0; i < HX_CRITERIA_SIZE; i++) {
		...
		ret = HX_INSPECT_EOTHER;
		E("%s,fail in line%d error code=%d\n", __func__, __LINE__, ret);
		goto CRITERIA_ARRAY_ALLOC_FAIL;
		}
	}
	...
	if( ret == HX_INSPECT_OK)
		g_flag_self_test_data_init = true;
	else
		g_flag_self_test_data_init = false;
	return ret;
