...
#if defined(HX_TP_USB_NOTIFIER) && defined(HX_USB_DETECT_GLOBAL)
...
static unsigned long himax_tp_headset_flag = 0;
#endif

#if defined(HX_AUTO_UPDATE_FW) || defined(HX_ZERO_FLASH)
char *i_CTPM_firmware_name = "tp/19661/Himax_firmware.bin";
char *i_CTPM_firmware_name_signed = "tp/19661/Himax_firmware_signed.bin";
char *i_CTPM_firmware_name_sort = "tp/19661/Himax_firmware_sort.bin";
...
#ifdef HX_HEADSET_DETECT_GLOBAL
bool himax_headset_mode_set(void);
#endif
									...
									size_t len, loff_t *pos)
{
	...
	if (buf_tmp[0] == '0') {
		himax_in_parse_assign_cmd(fw_oppo_tp_direction_0_pwd, buf_tmp, sizeof(buf_tmp));
		g_core_fp.fp_register_write(pfw_op->addr_oppo_tp_direction/*0x10007f3c*/, FOUR_BYTE_DATA_SZ, buf_tmp, 0);
		I("%s: oppo_tp_direction disable .\n", __func__);
	}
	else if(buf_tmp[0] == '1')
	{
		himax_in_parse_assign_cmd(fw_oppo_tp_direction_1_pwd, buf_tmp, sizeof(buf_tmp));
		g_core_fp.fp_register_write(pfw_op->addr_oppo_tp_direction, FOUR_BYTE_DATA_SZ, buf_tmp, 0);
		I("%s: oppo_tp_direction enable .\n", __func__);
	}
	else if(buf_tmp[0] == '2')
	{
		himax_in_parse_assign_cmd(fw_oppo_tp_direction_2_pwd, buf_tmp, sizeof(buf_tmp));
		g_core_fp.fp_register_write(pfw_op->addr_oppo_tp_direction, FOUR_BYTE_DATA_SZ, buf_tmp, 0);
		I("%s: oppo_tp_direction enable .\n", __func__);
	}
	else {
...
static ssize_t himax_incell_panel_read(struct file	*file, char	__user *user_buf,	size_t count,	loff_t *ppos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		if(strncmp (private_ts->chip_name,  HX_83112A_SERIES_PWON,  30))
		{
			temp_data=0;
		}
		else
...
*/
		...
		if (val == pfw_op->data_selftest_pass[0]) {
			ret += snprintf(temp_buf + ret, count - ret, "0 error(s). All test passed.\n");
		} else {
			ret += snprintf(temp_buf + ret, count - ret, "error code=0x%02X, Test Fail!\n", val);
		}
...
static ssize_t himax_baseline_test_write(struct file *file, const char *buff,
								size_t count, loff_t *pos)
{
	char buf[80] = {0};
	...
	I("%s: hx_ctpmodule = %d.\n", __func__, hx_ctpmodule);
	return count;
}

static ssize_t himax_blackscreen_test_read(struct file *file, char  *buf, size_t count, loff_t *pos)
{
	int val = 0x00;
    int ret = 0;
    int retry = 20;
    ...
    *pos += count;
    ...
    if (!ts->selftest_flag) {
		I("ts->gesture_test.flag ==== %d\n",ts->selftest_flag);
		return 0;
	}
    ptr = kzalloc(count, GFP_KERNEL);
    if (ptr == NULL) {
        E("failed to alloc ptr memory\n");
        return 0;
    }
...
static ssize_t himax_blackscreen_test_write(struct file *file, const char  *userbuf, size_t count, loff_t *pos)
{
    int value = 0;
	char buf[4] = {0};
...
static struct file_operations himax_proc_baseline_test_ops = {
	...
	.write = himax_baseline_test_write,
};

static struct file_operations himax_black_screen_test_fops = {
	.owner = THIS_MODULE,
...
/*
static ssize_t himax_double_tap_en_write(struct file *file, const char *buff,
								size_t len, loff_t *pos)
{
	struct himax_ts_data *ts = private_ts;
	char buf[80] = {0};
...
int himax_common_proc_init(void)
{
	...
	himax_proc_baseline_test_file = proc_create(HIMAX_PROC_BASELINE_TEST_FILE, (S_IRUGO), himax_touch_proc_dir, &himax_proc_baseline_test_ops);
	if (himax_proc_baseline_test_file == NULL) {
		E(" %s: proc baseline_test file create failed!\n", __func__);
		goto fail_oppo_0;
	}
	himax_proc_baseline_test_file = proc_create(HIMAX_BLACKSCREEN_TEST, (S_IRUGO | S_IWUGO), himax_touch_proc_dir, &himax_black_screen_test_fops);
	if (himax_proc_baseline_test_file == NULL) {
...
#ifdef HX_SMART_WAKEUP
#ifdef HX_GESTURE_TRACK
	himax_proc_coor_file = proc_create(HIMAX_PROC_COOR_FILE, (S_IWUSR | S_IRUGO),
	...
	himax_proc_oppo_tx_hop_file = proc_create(HIMAX_PROC_OPPO_TX_HOP_FILE, (S_IWUSR | S_IRUGO | S_IWUGO),
								  himax_touch_proc_dir, &himax_proc_oppo_tx_hop_ops);
	...
	return 0;
fail_oppo_11:
	remove_proc_entry(HIMAX_PROC_OPPO_TX_HOP_FILE, himax_touch_proc_dir);
fail_oppo_10:
...
fail_1:
#endif
	remove_proc_entry(HIMAX_BLACKSCREEN_TEST, himax_touch_proc_dir);
...
static int himax_wake_event_parse(struct himax_ts_data *ts, int ts_status)
{
	...
	if (buf[GEST_PTLG_ID_LEN] == GEST_PTLG_HDR_ID1 &&
		...
		for (i = 0; i < 14; i++) {
			I("buf[%d]=%d \n", GEST_PTLG_ID_LEN + 4 + i, buf[GEST_PTLG_ID_LEN + 4 + i]);
		}
...
void himax_cable_detect_func(bool force_renew)
{
	...
	USB_detect_flag = 0;/* must be set; upmu_is_chr_det(); */
#endif
...
#ifdef HX_ZERO_FLASH
		else if ( (hx_zero_event == length) && (ts_path == HX_REPORT_COORD)) {
...
#if defined(HX_HEADSET_DETECT_GLOBAL)
...
GET_TOUCH_FAIL:
#ifdef HX_RST_PIN_FUNC
	if( ts_path == HX_REPORT_SMWP_EVENT){
		I("%s: Now is gestrue mode ,no need reset.\n", __func__);	
	}else{
...
#ifdef HX_UPDATE_FW_NOTIFIER
static void himax_resume_workqueue(struct work_struct *work)
{
	I("%s: Entering! \n", __func__);
...
static void himax_esd_resume_func(void)
{
	...
	I("%s: Entering! \n", __func__);
#ifdef HX_ESD_RECOVERY
	if (!mutex_trylock(&private_ts->resume_update_fw_lock))
	{
		I("%s: already in esd resume! \n", __func__);
		return;
	}
	himax_esd_hw_reset();
	mutex_unlock(&private_ts->resume_update_fw_lock);
#endif
	I("%s: end! \n", __func__);
}
...
#ifndef HX_TP_HEADSET_NOTIFIER
	himax_tp_headset_flag = 0;// this flag need be set with some func.
	if(himax_tp_headset_flag == himax_tp_headset_state)
		return false;
	else
		himax_tp_headset_state = himax_tp_headset_flag;
#endif
...
int himax_chip_common_resume(struct himax_ts_data *ts)
{
	...
	g_core_fp.fp_resend_cmd_func(ts->suspended);
#ifdef HX_ZERO_FLASH
	//ts->gesture_resend_stus = 0;
	while( retry < 2 ){
		g_core_fp.fp_register_read(pdriver_op->addr_fw_define_2nd_flash_reload, FOUR_BYTE_DATA_SZ, data, 0);
		I("%s: data[0]=%X,data[1]=%X\n", __func__,data[0],data[1]);
		if(data[1] == 0x72 && data[0] == 0xC0) {
			retry = 0;
			I("%s: No need to update FW!\n",__func__);
			break;
		}else {
			retry++;	
			I("%s: retry=%d\n", __func__,retry);
	...
	if( retry > 0 || private_ts->HW_Download_flag == false || private_ts->CONFIG_Download_flag == false )
	{		
		I("It will update fw  in zero flash mode!\n");
		result = g_core_fp.fp_0f_operation_dirly();
		if (result) {
			E("Something is wrong! Skip Update with zero flash!\n");
			goto ESCAPE_0F_UPDATE1;
		}
		g_core_fp.fp_reload_disable(0);
		g_core_fp.fp_sense_on(0x00);
