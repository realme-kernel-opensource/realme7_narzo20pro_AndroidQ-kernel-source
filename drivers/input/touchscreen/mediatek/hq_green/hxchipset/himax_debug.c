...
const struct firmware *fw_image = NULL;
...
extern char *i_CTPM_firmware_name_signed;
...
static ssize_t himax_int_en_write(struct file *file, const char *buf,
									size_t len, loff_t *pos)
{
	struct himax_ts_data *ts = private_ts;
	int ret = 0;
...
static void himax_oppo_raw_data_read(int ts_diag_cmd)
{
	...
	I("%s,Start get Delta from DSRAM\n", __func__);
	ts->diag_cmd = ts_diag_cmd;
	diag_cmd = ts->diag_cmd % 10;
...
#define DIFF_VER_UPDT 0
#define FORE_UPDT 1
#define SIGNED_FW_UPDT 2
#define SAME_VER 0
...
{
	char buf_tmp[12] = {0};
	unsigned char fw_update_mode = FORE_UPDT;
	int ret = NO_ERR;
	int file_FW_VER = 0;
	int file_CFG_VER = 0;
	int file_CID_VER = 0;
...
#ifdef HX_ZERO_FLASH
	fw_update_mode = buf_tmp[0] - '0';

	switch (fw_update_mode) {
		case SIGNED_FW_UPDT:
			I("SIGNED_FW_UPDT\n");
			ret = request_firmware (&fw_image, i_CTPM_firmware_name_signed, private_ts->dev);//need signed fw parsing func instead of request_firmware func.
		case DIFF_VER_UPDT:
			if(fw_update_mode != SIGNED_FW_UPDT){
				I("DIFF_VER_UPDT\n");
				ret = request_firmware (&fw_image, i_CTPM_firmware_name, private_ts->dev);
			}
			if (ret < 0 || fw_image == NULL) {
				E("err = %d\n", ret);
				return len;
			}
			himax_int_enable(0);
			...
			if(i_CTPM_FW != NULL){
				...
				file_FW_VER = (i_CTPM_FW[FW_VER_MAJ_FLASH_ADDR] << 8) | i_CTPM_FW[FW_VER_MIN_FLASH_ADDR];
				file_CFG_VER = (i_CTPM_FW[CFG_VER_MAJ_FLASH_ADDR] << 8) | i_CTPM_FW[CFG_VER_MIN_FLASH_ADDR];
				file_CID_VER = (i_CTPM_FW[CID_VER_MAJ_FLASH_ADDR] << 8) |  i_CTPM_FW[CID_VER_MIN_FLASH_ADDR];
				I("file_FW_VER = 0x%2.2X, file_CFG_VER = 0x%2.2X, file_CID_VER = 0x%2.2X\n", file_FW_VER,file_CFG_VER,file_CID_VER);
				kfree(i_CTPM_FW);
			}
			ret = ((file_FW_VER != ic_data->vendor_fw_ver) ||
				 (file_CFG_VER != ((ic_data->vendor_touch_cfg_ver << 8) | ic_data->vendor_display_cfg_ver)) ||
				 (file_CID_VER != ((ic_data->vendor_cid_maj_ver << 8) | ic_data->vendor_cid_min_ver)));
			if(ret == SAME_VER) {
				I("SAME_VER, NO UPDATE!\n");
				break;
			}
		case FORE_UPDT:
			if(fw_update_mode != SIGNED_FW_UPDT && fw_update_mode != DIFF_VER_UPDT ){
				I("FORE_UPDT\n");
			}
...
int himax_touch_proc_init(void)
{
	...
	if (himax_proc_dbg_info_dir != NULL) {
		himax_proc_delta_file = proc_create(HIMAX_PROC_DELTA_FILE, (S_IWUSR | S_IRUGO),
		...
		himax_proc_baseline_file = proc_create(HIMAX_PROC_BASELINE_FILE, (S_IWUSR | S_IRUGO),
		...
		himax_proc_data_limit_file = proc_create(HIMAX_PROC_DATA_LIMIT_FILE, (S_IWUSR | S_IRUGO),
									  himax_proc_dbg_info_dir, &himax_proc_data_limit_ops);
...
fail_oppo_7:
	remove_proc_entry(HIMAX_PROC_DATA_LIMIT_FILE, himax_proc_dbg_info_dir);
fail_oppo_3_4:
