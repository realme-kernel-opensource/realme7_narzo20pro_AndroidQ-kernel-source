...
static ssize_t regulator_disable_set(struct device *dev,
                struct device_attribute *attribute, const char *buffer, size_t count)
{
        int op = 0;
        int rc = 0;
...
static DEVICE_ATTR(regulator_disable, S_IWUSR, NULL, regulator_disable_set);
...
static struct attribute *attributes[] = {
        ...
        &dev_attr_regulator_disable.attr,
...
static int fpc1020_irq_probe(struct platform_device *pldev)
{
        ...
        rc = gpio_direction_output(fpc1020->vdd_en_gpio, 0);
        if (rc < 0) {
                dev_err(fpc1020->dev, "gpio_direction_output (vdd_en) failed.\n");
                goto ERR_AFTER_WAKELOCK;
        }
