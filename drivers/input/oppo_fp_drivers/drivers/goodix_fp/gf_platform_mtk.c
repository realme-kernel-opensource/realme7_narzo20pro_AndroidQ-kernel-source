...
#ifdef NEMO_GF_LDO_GPIO //19661 18601 20682 Don't need
...
#endif
	pr_err("%s Fingerprint power on enter  \n",__func__);
	rc = regulator_enable(gf_regulator);
	pr_err("%s regulator_enable value = %d \n",__func__, rc);
	pr_info("---- power on ok ----\n");
...
{
	int rc = 0;
	pr_err("%s Fingerprint power off enter  \n",__func__);
