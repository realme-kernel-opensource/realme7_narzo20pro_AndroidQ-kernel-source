...
struct oppo_rk826_device {
	struct i2c_client *client;
	struct device *dev;
};

static struct oppo_rk826_device *rk826_chip = NULL;
...
static bool rk826_mcu_is_online(struct oppo_vooc_chip *chip)
{
        u8 value = 0;
        int rc = 0;
        bool is_online = true;
	...
	chip->client = rk826_chip->client;
        ...
        rc = oppo_i2c_dma_read(chip->client, 0x52f8, 1, &value);
        if (rc < 0) {
...
#ifdef ODM_HQ_EDIT
/*wangtao@ODM.BSP, 2020/06/15, add for after evt2 vooc hardware*/
int get_hwpcb_version(void)
{
	int pcb;
	pcb = get_PCB_Version();
...
#endif
int rk826_get_firmware(struct oppo_vooc_chip *chip)
{
	if (!chip)
		return -1;
...
struct oppo_vooc_operations oppo_rk826_ops = {
	...
	.get_firmware = rk826_get_firmware,
	.get_mcu_online = rk826_mcu_is_online,
};
