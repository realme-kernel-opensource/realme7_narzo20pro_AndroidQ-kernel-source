...
struct oppo_op10_device {
	struct i2c_client *client;
	struct device *dev;
};
static struct oppo_op10_device *op10_chip = NULL;
...
static bool op10_mcu_is_online(struct oppo_vooc_chip *chip)
{
        u8 value = 0;
        int rc = 0;
        bool is_online = true;
	...
	chip->client = op10_chip->client;
        ...
        rc = oppo_vooc_i2c_read(chip->client, 0x08, 1, &value);
        if (rc < 0) {
		...
		if (value == 0x02) {
			is_online = true;
        		chg_err("op10 detected, register 0x08: 0x %x\n", value);
		}
	}
...
int op10_get_firmware(struct oppo_vooc_chip *chip)
{
	if (!chip)
		return -1;
...
struct oppo_vooc_operations oppo_op10_ops = {
	...
	.get_firmware = op10_get_firmware,
	.get_mcu_online = op10_mcu_is_online,
};
