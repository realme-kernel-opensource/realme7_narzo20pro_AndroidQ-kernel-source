...
/*wangtao@ODM.HQ.BSP.CHG 2020/03/13 add charge type*/
...
int bq2589x_set_input_current_limit(struct bq2589x *bq, int curr)
{
	...
	bq2589x_update_bits(bq, BQ2589X_REG_00, BQ2589X_IINLIM_MASK, 
...
static int bq2589x_get_charger_type(struct bq2589x *bq, enum charger_type *type)
{
	...
	pr_err("bq2589x_get_charger_type:%d\n",vbus_stat);
	switch (vbus_stat) {
...
#ifdef ODM_HQ_EDIT
/*wangtao@ODM_HQ.BSP.CHG 2020/03/14, disable charger ic system reset function*/
	bq2589x_update_bits(bq, BQ2589X_REG_09, 
						BQ2589X_SYSTEM_OFF_MASK, BQ2589X_SYSTEM_OFF);
#endif
	return 0;
}
...
static int _bq2589x_get_ichg(struct bq2589x *bq, u32 *curr)
{
	...
	printk("wangtao _bq2589x_get_ichg =%d\n",ichg);
	return ret;
}
...
int oppo_bq2589x_set_ichg(int cur)
{
	...
	u32 temp_uA;
	int ret = 0;
	printk("wangtao oppo_bq2589x_set_ichg=%d\n",cur);
	if (g_bq->oppo_chg_type != POWER_SUPPLY_TYPE_UNKNOWN) {
		if(cur <= 1000){   // <= 1A
			ret = bq2589x_set_chargecurrent(g_bq, cur);
			chip->sub_chg_ops->charging_current_write_fast(0);//wangtao
		} else {
			...
			uA-=temp_uA;
			printk("wangtao oppo_bq2589x_set_ichg subchg =%d\n",uA);
			chip->sub_chg_ops->charging_current_write_fast(uA/1000);//wangtao
...
void oppo_bq2589x_set_mivr(int vbatt)
{
    ...
    if(uV<4200000)
     ...
     bq2589x_set_input_volt_limit(g_bq, uV / 1000);
}
...
int oppo_bq2589x_set_aicr(int current_ma)
{
	...
	/*if(chip->is_double_charger_support){
		if(current_ma>1000){
			bq2589x_set_input_current_limit(g_bq, current_ma*current_percent/100);
			chip->sub_chg_ops->input_current_write(current_ma*(100-current_percent)/100);
			chip->sub_chg_ops->charging_enable();
		}else{
			bq2589x_set_input_current_limit(g_bq, current_ma);
		}
	}else{
		bq2589x_set_input_current_limit(g_bq, current_ma);
	}
	...
	if (g_bq->pre_current_ma == current_ma) {
		dev_info(g_bq->dev, "%s sdp current limit=%d\n", __func__,g_bq->pre_current_ma);
		if (g_bq->pre_current_ma < 1000)
	...
	bq2589x_set_input_volt_limit(g_bq, 4500);
...
int oppo_bq2589x_charging_disable(void)
{
	...
	if(g_bq->hvdcp_can_enabled){
		//bq2589x_i2c_update_bits(g_bq, bq2589x_REG_CHG_DPDM2, 0x80, 0xE0);
...
int oppo_bq2589x_chg_get_pe20_type(void)
{
	struct charger_manager *info = NULL;
	...
	if(disable_PE){
		dev_info(g_bq->dev, "%s:disable_PE\n", __func__);
		return false;
	}
...
int oppo_bq2589x_chg_pe20_setup(void)
{
	struct charger_manager *info = NULL;
...
int oppo_bq2589x_chg_reset_pe20(void)
{
	struct charger_manager *info = NULL;
...
bool oppo_bq2589x_get_pd_type(void)
{
	struct charger_manager *info = NULL;
...
int oppo_bq2589x_pd_setup (void)
{
	struct oppo_chg_chip *chip = g_oppo_chip;
	struct charger_manager *info = NULL;
...
int oppo_bq2589x_chg_set_high_vbus(bool en)
{
	...
	if(subtype==CHARGER_SUBTYPE_PD){
	  if(en){
		...
		oppo_pdc_setup(info,5000);
		dev_info(g_bq->dev, "%s: set pd to 5V", __func__);
	  }
	  ...
	  if(!en){
	  	dev_info(g_bq->dev, "%s: set pe20 to 5V", __func__);
		mtk_pe20_reset_ta_vchr(info);
	  }
	}else{
...
struct oppo_chg_operations  oppo_chg_bq2589x_ops = {
	...
	.input_current_write = oppo_bq2589x_set_aicr,
...
//	void (*check_is_iindpm_mode) (void);
    .oppo_chg_get_pd_type = oppo_bq2589x_get_pd_type,
    .oppo_chg_pd_setup = oppo_bq2589x_pd_setup,
	...
	.oppo_chg_get_pe20_type = oppo_bq2589x_chg_get_pe20_type,
	.oppo_chg_pe20_setup = oppo_bq2589x_chg_pe20_setup,
	.oppo_chg_reset_pe20 = oppo_bq2589x_chg_reset_pe20,
...
#ifdef ODM_HQ_EDIT
//Junbo.Guo@ODM_HQ.BSP.CHG, 2019/12/20, Modify for subcharger
...
#ifdef ODM_HQ_EDIT
/*Sidong.Zhao@ODM_HQ.BSP.CHG 2019/11/4,for detect hvdcp*/
	bq->chg_consumer =
