...
* Revision 1.0    2018-11-09   Jianchao.Shi@PSW.BSP.CHG   	 for newCreated architecture
*************************************************************************************/
...
static int oppo_mt6360_input_current_limit_write(int value)
{
	...
	if (g_oppo_chip->chg_ops->oppo_chg_get_pd_type) {
		if (g_oppo_chip->chg_ops->oppo_chg_get_pd_type() == true) {
			//rc = oppo_pdc_get(&vbus_mv, &ibus_ma);   zhangchao modify?
...
static int oppo_mt6360_enable_charging(void)
{
	...
	printk(KERN_ERR "[OPPO_CHG][%s]: zhangchao oppo_mt6360_enable_charging!begin\n", __func__); //zhangchao add
	if (!g_oppo_chip) {
		printk(KERN_ERR "[OPPO_CHG][%s]: oppo_chip not ready!\n", __func__);
		return 0;
	}
	...
	printk(KERN_ERR "[OPPO_CHG][%s]: zhangchao oppo_mt6360_enable_charging!end\n", __func__); //zhangchao add
...
static int oppo_mt6360_disable_charging(void)
{
	...
	printk(KERN_ERR "[OPPO_CHG][%s]: zhangchao oppo_mt6360_disable_charging!begin\n", __func__); //zhangchao add
	if (!g_oppo_chip) {
		printk(KERN_ERR "[OPPO_CHG][%s]: oppo_chip not ready!\n", __func__);
		return 0;
	}
	...
	printk(KERN_ERR "[OPPO_CHG][%s]: zhangchao oppo_mt6360_disable_charging!end\n", __func__); //zhangchao add
...
static int mt_vadc_read()
{
	...
	printk("%s:enter\n", __func__);
	channel = iio_channel_get(chip->dev, "chargerID-channel");
	if (IS_ERR(channel)) {
		ret = PTR_ERR(channel);
	...
	printk("[%s]charger_id_voltage is %d\n", __func__, id_volt);

	return id_volt;
}
//extern bool g_ignore_usb; zhangchao
...
static int mt_get_chargerid_volt(void)
{
	...
	if (is_support_chargerid_check() == true) {
		...
		chargerid_volt = mt_vadc_read();//get the charger id volt zhangchao
...
static int oppo_usb_switch_gpio_gpio_init(void)
{
	...
	if (IS_ERR_OR_NULL(chip->normalchg_gpio.dischg_disable)) {
		chg_err("get charging_enable fail\n");
		return -EINVAL;
	}
	...
	if (IS_ERR_OR_NULL(chip->normalchg_gpio.dischg_enable)) {
		chg_err("get charging_disable fail\n");
		return -EINVAL;
	}
...
static int oppo_chg_chargerid_parse_dt(struct oppo_chg_chip *chip)
{
	...
	chip->normalchg_gpio.dischg_gpio =
			of_get_named_gpio(node, "qcom,charging_en-gpio", 0);
	if (chip->normalchg_gpio.dischg_gpio <= 0) {
		chg_err("Couldn't read charging_en-gpio rc=%d, charging_en-gpio:%d\n",
				rc, chip->normalchg_gpio.dischg_gpio);
	} else {
		if (gpio_is_valid(chip->normalchg_gpio.dischg_gpio)) {
			rc = gpio_request(chip->normalchg_gpio.dischg_gpio, "charging_en-gpio");
			if (rc) {
				chg_err("unable to request charging_en-gpio:%d\n",
						chip->normalchg_gpio.dischg_gpio);
			} else {
				rc = oppo_usb_switch_gpio_gpio_init();
				if (rc)
					chg_err("unable to init charging_en-gpio%d\n",
							chip->normalchg_gpio.dischg_gpio);
			}
		}
		chg_err("charging_en-gpio:%d\n", chip->normalchg_gpio.dischg_gpio);
	}
	//zhangchao add
	...
	enum power_supply_property psp)
{
	...
	switch (psp) {
		case POWER_SUPPLY_PROP_ONLINE:     //zhangchao add
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_TYPEC_SBU_VOLTAGE:
			//val->intval = oppo_get_typec_sbu_voltage(); zhangchao
			break;
		case POWER_SUPPLY_PROP_WATER_DETECT_FEATURE:
			//val->intval = oppo_get_water_detect();zhangchao
			break;
	...
	enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	switch (psp) {
		case POWER_SUPPLY_PROP_WATER_DETECT_FEATURE:
			...
			if (val->intval == 0) {
				//oppo_set_water_detect(false); zhangchao
			} else {
				//oppo_set_water_detect(true); zhangchao
			}
			break;
		default:
...
static enum power_supply_property battery_properties[] = {
	...
	//POWER_SUPPLY_PROP_MMI_CHARGING_ENABLE,        /*add for MMI_CHG_TEST*/zhangchao
#ifdef CONFIG_OPPO_CHARGER_MTK
...
static int oppo_mt6360_pd_setup(void)
{
	...
	//ret = oppo_pdc_setup(&vbus_mv, &ibus_ma);  zhangchao modify?
...
/* Jianchao.Shi@BSP.CHG.Basic, 2018/11/09, sjc Add for charging*/
	...
	oppo_chg_wake_update_work();  //zhangchao temp add //
	chr_err("%s: zhangchao starts\n", __func__);
	pr_err("zhangchao 2 mtk_charger_probe\n");
#endif
	...
	mtk_pdc_init(info);
	chr_err("%s: zhangchao1 starts\n", __func__);
	pr_err("zhangchao 3 mtk_charger_probe\n");
...
/* Jianchao.Shi@BSP.CHG.Basic, 2018/11/09, sjc Add for charging*/
	...
	pr_err("zhangchao 1 mtk_charger_probe sucess\n");
	//charger_xlog_printk(CHG_LOG_CRTI, "  ee zhangchao mtk_charger_probe sucess\n");
	//zhangchao add
...
//module_init(mtk_charger_init); zhangchao add
