...
static int stm8s_fw_update(struct oppo_vooc_chip *chip)
{
	...
	chg_debug("stm8s_fw_update 1  zhangchao fw_update\n");  //zhangchao add debug
...
update_fw:
        ...
        for (i = 0;i < ERASE_COUNT;i++) {
                /*first:set address*/
                /*rc = i2c_smbus_write_i2c_block_data(chip->client, 0x01, 2, &addr_buf[0]);*/
		chg_debug("stm8s_fw_update 2  zhangchao fw_update\n");  //zhangchao add debug
                rc = oppo_vooc_i2c_write(chip->client, 0x01, 2, &addr_buf[0]);
                if (rc < 0) {
	...
	chg_debug("stm8s_fw_update 3  zhangchao fw_update\n");  //zhangchao add debug

        /*fw check begin:read data from stm1503/1508, and compare it with Stm8s_firmware_data[]*/
        rc = stm8s_fw_check_frontline(chip);
        if (rc == FW_CHECK_FAIL) {
		chg_debug("stm8s_fw_update 4  zhangchao fw_update\n");  //zhangchao add debug
                download_again++;
                if (download_again > 3) {
                        goto update_fw_err;
                }
		chg_debug("stm8s_fw_update 5  zhangchao fw_update\n");  //zhangchao add debug
	...
	chg_debug("stm8s_fw_update 6  zhangchao fw_update\n");  //zhangchao add debug
...
static void stm8s_fw_check_then_recover(struct oppo_vooc_chip *chip)
{
	...
	chg_debug("stm8s_fw_check_then_recover  zhangchao fw_update\n");  //zhangchao add debug
        ...
        if (oppo_is_power_off_charging(chip) == true || oppo_is_charger_reboot(chip)== true) {
        	...
        	chg_debug("stm8s_fw_check_then_recover 1  zhangchao fw_update\n");  //zhangchao add debug
                opchg_set_clock_active(chip);
                chip->mcu_boot_by_gpio = true;
                msleep(10);
                opchg_set_reset_active(chip);
			...
			chg_debug("stm8s_fw_check_then_recover 2  zhangchao fw_update\n");  //zhangchao add debug
                } else {
                        chip->vooc_fw_check = true;
...
**    Modify for vooc batt 4.40   */
		...
		if(chip->vooc_fw_update_newmethod) {
			...
			oppo_vooc_fw_update_work_init(chip);
			chg_debug("stm8s probe zhangchao fw_update\n");  //zhangchao add debug
		}
        oppo_vooc_init(chip);
