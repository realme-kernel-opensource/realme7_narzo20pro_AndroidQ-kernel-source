...
static int iWriteCAM_CAL(u16 a_u2Addr, u32 a_u4Bytes, u8 *puDataInBytes)
{
	...
	if (a_u4Bytes + 2 > 8) {
		CAM_CALDB("[P24C64E] exceed I2c-mt65xx.c 8 bytes limitation (include address 2 Byte)\n");
		return -1;
	}
	...
	if (i4RetValue != (a_u4Bytes + 2)) {
		CAM_CALDB("[P24C64E] I2C write  failed!!\n");
		return -1;
	}
...
static int iReadCAM_CAL(u16 a_u2Addr, u32 ui4_length, u8 *a_puBuff)
{
	...
	if (ui4_length > 8) {
		CAM_CALDB("[P24C64E] exceed I2c-mt65xx.c 8 bytes limitation\n");
		return -1;
	}
...
static int iWriteData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char *pinputdata)
{
	...
	CAM_CALDB("[P24C64E] iWriteData\n");

	if (ui4_offset + ui4_length >= 0x2000) {
		CAM_CALDB("[P24C64E] Write Error!! P24C64E not supprt address >= 0x2000!!\n");
		return -1;
	}
	...
	CAM_CALDB("[P24C64E] iWriteData u4CurrentOffset is %d\n", u4CurrentOffset);
	...
	CAM_CALDB("[P24C64E] iWriteData done\n");
...
static int iReadData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char *pinputdata)
{
	...
	if (ui4_offset + ui4_length >= 0x2000) {
		CAM_CALDB("[P24C64E] Read Error!! P24C64E not supprt address >= 0x2000!!\n");
		return -1;
	}
	...
	do {
		if (i4ResidueDataLength >= 8) {
			...
			if (i4RetValue != 0) {
				CAM_CALDB("[P24C64E] I2C iReadData failed!!\n");
				return -1;
			}
			u4IncOffset += 8;
	...
	unsigned char *data, unsigned int size)
{
	...
	if (!xtalk_readed) {
	    ...
	    /*Cong.Zhou@ODM_HQ Cam.Drv 201901205 for improve open front camera speed*/
	    int i =0;
	    int read_data_checksum=0;
	    char e2prom_data_checksum=0;
	    #endif
		...
		if (Sensor_ID == EEPROM_SENSORID_S5K3P9SP) {
			...
			if (xtalk_flag == 1) {
				printk("s5k3p9sp's Sensor_ID = 0x%x to read 4cell algorithm\n", Sensor_ID);
				iReadData(S5K3P9SP_XTALK_START_ADDR, S5K3P9SP_XTALK_DATA_SIZE, s5k3p9sp_data_xtalk);
				#ifndef ODM_HQ_EDIT
				/*Cong.Zhou@ODM_HQ Cam.Drv 201901205 for improve open front camera speed*/
				for (i=0;i<S5K3P9SP_XTALK_DATA_SIZE;i++)
				{
				     read_data_checksum = s5k3p9sp_data_xtalk[i]+read_data_checksum;
				     //printk("s5k3p9sp_data_xtalk[%d] = %d\n",i,s5k3p9sp_data_xtalk[i]);
			    }
				iReadData(0x1C01, 1, &e2prom_data_checksum);
				printk("s5k3p9sp_data_xtalk checksum: read_data_checksum:%d e2prom_data_checksum:%d\n",read_data_checksum%255,e2prom_data_checksum);
				#endif
				xtalk_readed = 1;
			} else {
				printk("Warning, s5k3p9sp'otp not have cross-talk calibration data !\n");
			}
		}
...
unsigned int p24c64e_read_4cell_from_eeprom_s5k3p9sp(char *data)
{
	...
	if (data != NULL) {
		...
		pr_debug("s5k3p9sp read 4cell data[0]=%d, data[10]=%d, data[100]=%d, data[2047]=%d \n", data[2], data[12], data[102], data[2047]);
		for(i=0;i<20;i++)
		{
		   printk("s5k3p9sp read 4cell data[%d]=%d",i,data[i]);
		}
	}
	return 0;
}
...
/*#define P24C64E_DRIVER_ON*/
#ifdef P24C64E_DRIVER_ON
...
#endif
{
	...
	if (_IOC_DIR(a_u4Command) != _IOC_NONE) {
		...
		if (pBuff == NULL) {
			CAM_CALDB("[P24C64E] ioctl allocate mem failed\n");
			return -ENOMEM;
		}
		...
		if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
			if (copy_from_user((u8 *) pBuff, (u8 *) a_u4Param, sizeof(stCAM_CAL_INFO_STRUCT))) {
				...
				kfree(pBuff);
				CAM_CALDB("[P24C64E] ioctl copy from user failed\n");
				return -EFAULT;
			}
		}
	}
	...
	CAM_CALDB("[P24C64E] init Working buffer address 0x%8x  command is 0x%8x\n",
	...
	case CAM_CALIOC_G_READ:
		CAM_CALDB("[P24C64E] Read CMD\n");
#ifdef CAM_CALGETDLT_DEBUG
		do_gettimeofday(&ktv1);
#endif
		CAM_CALDB("[P24C64E] offset %d\n", ptempbuf->u4Offset);
		CAM_CALDB("[P24C64E] length %d\n", ptempbuf->u4Length);
		CAM_CALDB("[P24C64E] Before read Working buffer address 0x%8x\n", (u32)pWorkingBuff);

		/* i4RetValue = iReadData((u16)ptempbuf->u4Offset, ptempbuf->u4Length, pWorkingBuff); */
		CAM_CALDB("[P24C64E_CAM_CAL] After read Working buffer data  0x%4x\n", *pWorkingBuff);
		...
		if (ptempbuf->u4Offset == 0x800) {
			...
			*(u32 *)pWorkingBuff = 0x100CB032;
			...
			CAM_CALDB("[P24C64E] Shading CheckSum MSB=> %x %x\n", pWorkingBuff[0],
			pWorkingBuff[1]);
		} else {
			...
			while (readTryagain > 0) {
				...
				CAM_CALDB("[P24C64E] error (%d) Read retry (%d)\n", i4RetValue,
				readTryagain);
				if (i4RetValue != 0)
					readTryagain--;
				else
		...
		if (1) {
			...
			CAM_CALDB("[P24C64E]enter EEPROM_test function%d (%x)%x\n", ++loop,
			0x0000, get_byte);
	...
	if (_IOC_READ & _IOC_DIR(a_u4Command)) {
		...
		CAM_CALDB("[P24C64E] to user length %d\n", ptempbuf->u4Length);
		CAM_CALDB("[P24C64E] to user  Working buffer address 0x%8x\n", (u32)pWorkingBuff);
		if (copy_to_user((u8 __user *) ptempbuf->pu1Params, (u8 *)pWorkingBuff,
			ptempbuf->u4Length)) {
			kfree(pBuff);
			kfree(pWorkingBuff);
			CAM_CALDB("[P24C64E] ioctl copy to user failed\n");
			return -EFAULT;
		}
	}
...
static int CAM_CAL_Open(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	CAM_CALDB("[P24C64E_CAM_CAL] CAM_CAL_Open\n");
...
static inline int RegisterCAM_CALCharDrv(void)
{
	...
	if (alloc_chrdev_region(&g_CAM_CALdevno, 0, 1, CAM_CAL_DRVNAME)) {
		CAM_CALDB("[P24C64E_CAM_CAL] Allocate device no failed\n");

		return -EAGAIN;
	}
#else
	if (register_chrdev_region(g_CAM_CALdevno, 1, CAM_CAL_DRVNAME)) {
		CAM_CALDB("[P24C64E_CAM_CAL] Register device no failed\n");
	...
	if (g_pCAM_CAL_CharDrv == NULL) {
		...
		CAM_CALDB("[P24C64E_CAM_CAL] Allocate mem for kobject failed\n");
	...
	if (cdev_add(g_pCAM_CAL_CharDrv, g_CAM_CALdevno, 1)) {
		CAM_CALDB("[P24C64E_CAM_CAL] Attatch file operation failed\n");
...
static int CAM_CAL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	CAM_CALDB("[P24C64E_CAM_CAL] Attach I2C\n");
	/* spin_lock_init(&g_CAM_CALLock); */

	/* get sensor i2c client */
	spin_lock(&g_CAM_CALLock); /* for SMP */
	g_pstI2Cclient = client;
	g_pstI2Cclient->addr = P24C64E_DEVICE_ID >> 1;
	spin_unlock(&g_CAM_CALLock); /* for SMP */

	CAM_CALDB("[P24C64E_CAM_CAL] g_pstI2Cclient->addr = 0x%8x\n", g_pstI2Cclient->addr);
	/* Register char driver */
	i4RetValue = RegisterCAM_CALCharDrv();

	if (i4RetValue) {
		CAM_CALDB("[P24C64E_CAM_CAL] register char device failed!\n");
		return i4RetValue;
	}


	CAM_CALDB("[P24C64E_CAM_CAL] Attached!!\n");
	return 0;
}
...
static int __init CAM_CAL_i2C_init(void)
{
	...
	if (platform_driver_register(&g_stCAM_CAL_Driver)) {
		CAM_CALDB("failed to register P24C64E_CAM_CAL driver\n");
		return -ENODEV;
	}

	if (platform_device_register(&g_stCAM_CAL_Device)) {
		CAM_CALDB("failed to register P24C64E_CAM_CAL driver, 2nd time\n");
		return -ENODEV;
	}
