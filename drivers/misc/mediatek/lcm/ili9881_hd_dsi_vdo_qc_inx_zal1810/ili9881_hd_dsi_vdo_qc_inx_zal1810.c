 ...
 ** ODM_HQ_EDIT
 ** File: ili9881_hd_dsi_vdo_qc_inx_zal1810.c
...
static struct LCM_setting_table init_setting_vdo[] = {
	...
	{0xC7, 1,{0x05}},
	{0xFF, 3,{0x98,0x81,0x00}},
	{0x55, 0x01, {0x01}}, //level 1
	{0x53, 0x01, {0x24}}, //dimming disable
	//{0x68, 2,{0x04,0x00}},
    {0x35, 1,{0x00}},	
    {0x29, 1,{0x00}},
    {REGFLAG_DELAY, 20, {}}
...
#ifndef BUILD_LK
    register_device_proc("lcd", "ili9881_qc_inx", "ilitek");
#endif
}
...
//Jinzhu.Han@RM.MM.Display.Driver, 2019/01/10, Add for 3level cabc feature.
    LCM_LOGD("set the cabc_lastlevel = %d\n",cabc_lastlevel);
    switch (cabc_lastlevel) {
        ...
        case 3 :
            ...
            break;
        default :
            LCM_LOGD("default cabc lastlevel = 1\n");
            break;
    }
#endif
}
...
static void lcm_suspend(void)
{
    ...
    if(g_gesture <= 0) {
        ...
        SET_LCD_BIAS_EN(OFF, VSN_FIRST_VSP_AFTER, 6000);  //open lcd bias
        MDELAY(15);
    }
#endif
    //SET_RESET_PIN(0);
...
//bias end
    ...
    LCM_LOGI("%s,qc_ili9881c_id=0x%08x,version_id=0x%x\n",__func__, id, version_id);
    printf("%s,qc_ili9881c_id=0x%08x,version_id=0x%x\n",__func__, id, version_id);// id=0x00000098,version_id=0x81
    push_table(NULL,
        switch_table_page0,
...
{
    printk("%s,qc_ili9881c qc backlight: level = %d\n", __func__, level);
...
struct LCM_DRIVER ili9881_hd_dsi_vdo_qc_inx_zal1810_lcm_drv = {
    .name = "ili9881_qc_inx",
