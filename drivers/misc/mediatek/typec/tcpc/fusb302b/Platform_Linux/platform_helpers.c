		...
		void *usr_ctx, void *app_ctx)
{
	...
	case CC2_ORIENT:
		...
		//zhangchao@ODM_HQ.BSP.CHG 2020/03/16,add for typec_cc_orientation
		if (event == CC1_ORIENT)
			chip->tcpc->typec_polarity = false;
		else
			chip->tcpc->typec_polarity = true;
		pr_info("FUSB %s:zhangchao 0 event=0x%x,typec_polarity = %d\n", __func__, event,chip->tcpc->typec_polarity);
		break;
	case CC_NO_ORIENT:
