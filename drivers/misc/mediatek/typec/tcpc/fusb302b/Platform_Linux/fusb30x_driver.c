...
#include "../core/TypeC.h"      //zhangchao
...
/*int fusb30x_get_mode(struct tcpc_device *tcpc, int *typec_mode)
{
	...
	pr_info("%s - typec_mode:%d,sourceOrSink:%x\n",
		*typec_mode, chip->port.sourceOrSink);
	return 0;
}
                          ...
                          const struct i2c_device_id* id)
{
	...
	typec_desc->name = kzalloc(13, GFP_KERNEL);
	if (!typec_desc->name)
		return -ENOMEM;

	strcpy((char *)typec_desc->name, "type_c_port0");
	typec_desc->role_def = TYPEC_ROLE_TRY_SRC;
	...
	chip->tcpc = tcpc_device_register(&client->dev,
			typec_desc, &fusb30x_tcpc_ops, chip);
