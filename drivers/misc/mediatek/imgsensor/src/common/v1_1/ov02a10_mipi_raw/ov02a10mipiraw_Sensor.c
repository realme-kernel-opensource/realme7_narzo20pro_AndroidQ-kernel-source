...
static  imgsensor_info_struct imgsensor_info = {
	...
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_MONO, //b
			...
			*sensor_id = ((read_cmos_sensor(0x0200) << 8) | read_cmos_sensor(0x0300));
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if (deviceInfo_register_value == 0x00) {
					//register_imgsensor_deviceinfo("Cam_b_s", DEVICE_VERSION_OV02A10, imgsensor_info.module_id);  //compiler not found 20190812
