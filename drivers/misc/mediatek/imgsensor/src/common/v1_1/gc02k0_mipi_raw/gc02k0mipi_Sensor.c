...
#define DEVICE_VERSION_GC2385H    "gc2385h"
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if (deviceInfo_register_value == 0x00) {
					if (check_eeprom_awb_golden(*sensor_id, 0xA5)) {
						sprintf(sensor_name_golden_result, "%s_1", DEVICE_VERSION_GC2385H);
					} else {
						sprintf(sensor_name_golden_result, "%s_0", DEVICE_VERSION_GC2385H);
					}
...
UINT32 GC02K0_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
