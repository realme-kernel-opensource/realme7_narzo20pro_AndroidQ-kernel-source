...
#define EEPROM_SUB_QT_VENDOR_ID 0x05
#if MULTI_WRITE
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            if (vendorId == EEPROM_SUB_QT_VENDOR_ID) {
                *sensor_id = ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001)) + 2;
                if (*sensor_id == imgsensor_info.sensor_id) {
                    pr_info("s5k3l6_sub_qt Read sensor id success, sensorID: 0x%x vendorID=0x%x\n",
                            *sensor_id,vendorId);
                    return ERROR_NONE;
                }
                pr_info("s5k3l6_sub_qt Read sensor id fail, write_id:0x%x, sensor_id:0x%x\n",
                        imgsensor.i2c_write_id, *sensor_id);
            } else {
                pr_info("s5k3l6_sub_qt vendorId is not matched(0x05!=0x%x)\n", vendorId);
            }
            retry--;
        } while (retry > 0);
        i++;
    ...
    if ((*sensor_id != imgsensor_info.sensor_id)
        || (vendorId != EEPROM_SUB_QT_VENDOR_ID)) {
...
UINT32 S5K3L6_SUB_QT_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
