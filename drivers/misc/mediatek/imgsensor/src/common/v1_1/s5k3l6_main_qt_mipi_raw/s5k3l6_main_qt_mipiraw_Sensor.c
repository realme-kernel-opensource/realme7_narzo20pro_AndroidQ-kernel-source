...
#ifndef ODM_HQ_EDIT
/* Degao.Lan@HQ Cam.DRV 20200316 add for long exposure */
#define ODM_HQ_EDIT
#endif
...
#define EEPROM_MAIN_QT_VENDOR_ID 0x05
#if MULTI_WRITE
...
static struct imgsensor_struct imgsensor = {
        ...
        #ifdef ODM_HQ_EDIT
        /* Degao.Lan@HQ Cam.DRV 20200316 add for long exposure n+1*/
...
static void set_shutter(kal_uint32 shutter)
{
    ...
    if (shutter > 65530) {  //linetime=10160/960000000<< maxshutter=3023622-line=32s
        ...
        long_shutter = (shutter * 48) / (long_shutter_linelenght / 10); //line_lengthpckÒÑ¾­¸Ä±äÐèÒªÖØÐÂ¼ÆËãlongshuterµÄlinelength
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            if (vendorId == EEPROM_MAIN_QT_VENDOR_ID) {
                *sensor_id = ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001));
                if (*sensor_id == imgsensor_info.sensor_id) {
                    pr_info("s5k3l6_mian_qt Read sensor id success, sensorID: 0x%x vendorID=0x%x\n",
                            *sensor_id,vendorId);
                    return ERROR_NONE;
                }
                pr_info("s5k3l6_mian_qt Read sensor id fail, write_id:0x%x, sensor_id:0x%x\n",
                        imgsensor.i2c_write_id, *sensor_id);
            } else {
                pr_info("s5k3l6_mian_qt vendorId is not matched(0x05!=0x%x)\n", vendorId);
            }
            retry--;
        } while (retry > 0);
        i++;
    ...
    if ((*sensor_id != imgsensor_info.sensor_id)
        || (vendorId != EEPROM_MAIN_QT_VENDOR_ID)) {
                                  ...
                                  UINT8 *feature_para, UINT32 *feature_para_len)
{
        ...
        case SENSOR_FEATURE_GET_PERIOD:
                ...
                *feature_para_len = 4;
                break;
       /*Weifeng.Wang@ODM_HQ Cam.Drv, 20200426,add for monkey*/
...
UINT32 S5K3L6_MAIN_QT_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
