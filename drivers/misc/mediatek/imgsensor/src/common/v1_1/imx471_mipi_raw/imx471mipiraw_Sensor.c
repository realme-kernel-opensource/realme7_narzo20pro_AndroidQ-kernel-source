		...
		*sensor_id = return_sensor_id();
		if (*sensor_id == imgsensor_info.sensor_id) {
			...
			if (deviceInfo_register_value == 0x00) {
				if (check_eeprom_awb_golden(*sensor_id, 0xA8)) {
					sprintf(sensor_name_golden_result, "%s_1", DEVICE_VERSION_IMX471);
				} else {
					sprintf(sensor_name_golden_result, "%s_0", DEVICE_VERSION_IMX471);
				}
				register_imgsensor_deviceinfo("Cam_f", sensor_name_golden_result, imgsensor_info.module_id);
