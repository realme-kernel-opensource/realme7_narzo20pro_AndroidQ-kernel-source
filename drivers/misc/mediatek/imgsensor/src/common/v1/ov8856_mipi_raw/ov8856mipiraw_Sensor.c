...
#endif //VENDOR_EDIT
kal_uint16 addr_data_pair_dual_24fps_ov8856[] = {
	//2304x1728_24f
	0x0302, 0x35,
	0x0303, 0x00,
	0x3501, 0x9a,
...
kal_uint16 addr_data_pair_cap_24fps_ov8856[] = {
	0x0302, 0x35,
	0x0303, 0x00,
	0x3501, 0x9a,
	0x3502, 0x20,
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			pr_err("ov8856 Read sensor id,write_id:0x%x, id: 0x%x\n",
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				if ((read_cmos_sensor(0x302A)) == 0xB1) {
					...
					if (deviceInfo_register_value == 0x00) {
						//if (check_eeprom_awb_golden(*sensor_id, 0xA3)) {
						//	sprintf(sensor_name_golden_result, "%s_1", DEVICE_VERSION_OV8856);
						//} else {
						//	sprintf(sensor_name_golden_result, "%s_0", DEVICE_VERSION_OV8856);
						//}
						//register_imgsensor_deviceinfo("Cam_r1", sensor_name_golden_result, imgsensor_info.module_id);
