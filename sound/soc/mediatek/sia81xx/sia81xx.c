...
int sia81xx_power_on(void)
{
	...
	pr_info("%s: temp_sia81xx_dev=%p, scene=%d\n", __func__, temp_sia81xx_dev, temp_sia81xx_dev->scene);

	ret = sia81xx_resume(temp_sia81xx_dev);

	return ret;
}
EXPORT_SYMBOL(sia81xx_power_on);
...
int sia81xx_power_off(void)
{
	...
	ret = sia81xx_suspend(temp_sia81xx_dev);
	temp_sia81xx_dev = NULL;

	return ret;
}
EXPORT_SYMBOL(sia81xx_power_off);
//END
...
{	
	...
	mdelay(8);/* wait for owi reset */ // increase reset time
	...
	struct sia81xx_dev_s *sia81xx)
{
	...
	if(0 == sia81xx->disable_pin) {
		pr_info("[debug][%s] %s:%d enter\r\n", LOG_FLAG, __func__, __LINE__);
		/* power up chip */
	...
	const struct i2c_device_id *id)
{
	...
	pr_err("[ info][%s] %s: i2c addr = 0x%02x \r\n", 
	...
	if (CHIP_TYPE_UNKNOWN <= get_chip_id(sia81xx))
		return -ENODEV;
...
{	
	...
	pr_err("[ info][%s] %s: sia81xx driver version : %s \r\n", 
	...
	if (ret) {
		pr_err("[  err][%s] %s: i2c_add_driver error, ret = %d !!! \r\n",
			LOG_FLAG, __func__, ret);
		goto exit_get_chip_id_failed;
	}

	
	return 0;

exit_get_chip_id_failed:
